...
function checkAutomation(data) {
    ...
    if (data.rules.pm25) {
        if (avgData.pm25 > 25 && !data.devices.airPurifier.active) {
            addLog(`PM2.5 level high (${avgData.pm25}µg/m³) - Air Purifier activated`, 'warning');
        } else if (avgData.pm25 <= 12 && data.devices.airPurifier.active) {
            addLog(`PM2.5 level normalized (${avgData.pm25}µg/m³) - Air Purifier deactivated`, 'success');
        }
    }
    ...
    if (data.rules.co2) {
        if (avgData.co2 > 1000 && !data.devices.windowServo.active) {
            addLog(`CO₂ level high (${avgData.co2}ppm) - Window opened, Fan started`, 'warning');
        } else if (avgData.co2 <= 800 && data.devices.windowServo.active) {
            addLog(`CO₂ level normalized (${avgData.co2}ppm) - Window closed, Fan stopped`, 'success');
        }
    }
